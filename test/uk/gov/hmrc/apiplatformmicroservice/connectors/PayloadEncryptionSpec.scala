...
class PayloadEncryptionSpec extends UnitSpec with MockitoSugar {
  ...
  trait Setup {
    val mockPayloadEncryptionConfig = mock[PayloadEncryptionConfig]
    when(mockPayloadEncryptionConfig.jsonEncryptionKey).thenReturn("czV2OHkvQj9FKEgrTWJQZVNoVm1ZcTN0Nnc5eiRDJkY=")
    val payloadEncryption = new PayloadEncryption(new LocalCrypto(mockPayloadEncryptionConfig))
  }
