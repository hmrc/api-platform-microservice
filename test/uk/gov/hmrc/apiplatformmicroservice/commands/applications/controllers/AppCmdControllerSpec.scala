...
class AppCmdControllerSpec extends AsyncHmrcSpec with ApiDefinitionTestDataHelper with FixedClock {
  ...
  trait Setup
    ...
    val sandboxApplication      = buildSandboxApp().withId(sandboxApplicationId)
    val productionApplicationId = ApplicationId.random
    val productionApplication   = buildSandboxApp().withId(productionApplicationId).modify(_.copy(deployedTo = Environment.PRODUCTION))
