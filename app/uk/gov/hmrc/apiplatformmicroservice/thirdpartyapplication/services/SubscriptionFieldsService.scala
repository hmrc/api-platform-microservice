...
class SubscriptionFieldsService @Inject()(
  subsFieldsConnectorWrapper: EnvironmentAwareSubscriptionFieldsConnector
){
  def fetchFieldsValues(application: Application, fieldDefinitions: Seq[SubscriptionFieldsDomain.SubscriptionFieldDefinition], apiIdentifier: ApiIdentifier)(
      implicit hc: HeaderCarrier
  ): Future[Seq[SubscriptionFieldValue]] = {
    val connector = subsFieldsConnectorWrapper(application.deployedTo)
